...
SKIP_AWS_CHECK="false"                       # true なら AWS 認証 / CodeCommit 権限チェックを行わない
AUTO_SWITCH_ROLE="false"                     # true: 権限不足時に自動でスイッチロール / false: 警告して終了
SWITCH_ROLE_SCRIPT="${SWITCH_ROLE_SCRIPT:-}" # スイッチロール用シェル(別チーム提供)のパス。環境変数でも指定可。
                                             #   --auto-switch-role 指定時、または手動案内に使用する。
                                             #   source で呼び出して現在のシェルに認証情報を反映する。

# preflight で確定する内部状態（remote が CodeCommit かどうか）
IS_CODECOMMIT="false"
...
AWS 認証 / CodeCommit 権限関連:
  本スクリプトは（リモートが CodeCommit の場合）実行開始時に、
  ...
  --auto-switch-role      CodeCommit 権限が無い場合、終了せず別チーム提供のシェルを
                          source して自動的にスイッチロールします (既定: 警告して終了)。
  --switch-role-script <path>
                          スイッチロール用シェル（別チーム提供）のパス。
                          --auto-switch-role 時は source で呼び出してスイッチロールします。
                          省略時は手動でスイッチロールするよう案内します。
                          環境変数 SWITCH_ROLE_SCRIPT でも指定可。
...
# ---------------------------------------------------------------------------
# 4.5 AWS 認証 / CodeCommit 権限の事前チェック
#   ※ この節の関数は preflight() の後（REMOTE / IS_CODECOMMIT 確定後）に
#      aws_preflight() から呼び出される。git ラッパ git_r は §5 で定義。
# ---------------------------------------------------------------------------
...
preflight() {
  ...
  # CodeCommit リモートかどうかを判定する（AWS 認証 / 権限チェックの対象判定に使う）。
  ...
  if [[ "${remote_url}" == codecommit::* || "${remote_url}" == *git-codecommit.* ]]; then
    IS_CODECOMMIT="true"
    require_command aws
    log_debug "CodeCommit 形式の remote を検出しました。"
  fi

  # grc(git-remote-codecommit)形式の remote なら専用ヘルパも確認
  if [[ "${remote_url}" == codecommit::* ]]; then
    require_command git-remote-codecommit
    log_debug "grc 形式の remote を検出（git-remote-codecommit 確認済み）。"
  fi
}
